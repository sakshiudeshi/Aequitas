...
name = config.sensitive_param_name
cov = 0
...
global_iteration_limit = config.global_iteration_limit # need to be at least 1000 iterations in order to yield result
...
f = open(f'RetrainingInputs/{retraining_example_filename}', 'w')
...
class Local_Perturbation(object):
    ...
    def __call__(self, x):
        s = self.stepsize
        param_choice = np.random.choice(range(num_params))
        act = [-1, 1]
...
def evaluate_global(inp):
    ...
    if (abs(out0 - out1) > threshold and tuple(map(tuple, inp0)) not in global_disc_inputs):
        ...
        f.write(",".join(list(map(lambda x: str(x), inp0.tolist()[0]))) + "\n") # write inputs as they are generated
